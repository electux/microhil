...
static const char *const MICROHIL_VERSION = "microHIL-WIFI v1.0.0";
static const char *const MICROHIL_BOARD_ID = "mh:333:2023:0:wifi";
...
static void cmd_handle_blink(const char *cmd_str) {
  char ch_char = cmd_str[CMD_CHANNEL_CHAR_INDEX];
  uint32_t channel = ch_char - CMD_CHANNEL_MIN_CHAR;
  uint32_t on_time = 0, off_time = 0, count = 0;
...
static void cmd_handle_reset(const char *cmd_str) {
  (void)cmd_str;
  response_send("<mh#sys#system resetting...#end>");
  buzzer_beep_stop();
...
static const command_entry_t command_tbl[] = {
    ...
    {"mh#net#get#end", net_command_handle_get},
    {"mh#net#reset#end", net_command_handle_reset},
};
...
void command_dispatch(const char *cmd_str) {
  if (strncmp(cmd_str, "mh#net#set#ap#", 14) == 0) {
    net_command_handle_set_ap(cmd_str);
    return;
  }
  if (strncmp(cmd_str, "mh#net#set#sta#", 15) == 0) {
    net_command_handle_set_sta(cmd_str);
    return;
  }
