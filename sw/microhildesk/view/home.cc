...
namespace
{
    ...
    ///   home_toggle_channel_button - toggle button channel label 
    ///   home_timer_channel_label - label for timer channel
    ///   home_timer_channel_button - spinner channel button label
...
AppHome::AppHome()
{
    ...
    for(auto i = 0; i < home_num_channels; i++)
    {
        ...
        m_label_toggle_channels.emplace_back(
            home_toggle_channel_label + channel
        );
        m_box_channels[i].append(m_label_toggle_channels[i]);
        m_toggle_channels.emplace_back();
        m_toggle_channels[i].set_label(home_toggle_channel_button);
        m_box_channels[i].append(m_toggle_channels[i]);
        m_label_timer_channels.emplace_back(
            home_timer_channel_label + channel
        );
        ...
        m_toggle_timer_channels[i].set_label(home_timer_channel_button);
