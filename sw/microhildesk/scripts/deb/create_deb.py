...
Info
    A Python script for creating a Debian installation package (.deb) for microhildesk.
'''
...
__copyright__: str = '(C) 2026, https://vroncevic.github.io/microhil'
__credits__: list[str] = ['Vladimir Roncevic', 'Python Software Foundation']
__license__: str = 'https://github.com/vroncevic/microhil/blob/dev/LICENSE'
...
BINARY_PATH: Final[str] = os.path.join(BUILD_DIR, 'microhildesk')
LOGO_PATH: Final[str] = os.path.join(REPO_ROOT, 'docs', 'microhil_logo.png')
...
def create_control_file(
    ...
    """
    content = f"""Package: microhildesk
Version: {version}
Section: utils
Priority: optional
Architecture: {arch}
...
Description: Desktop GUI application for microHIL hardware controller
 A desktop GUI application built in C++ using gtkmm for orchestrating
 and controlling microHIL hardware controllers (relays, LED, buzzer, etc.)
...
Type=Application
Name=microHIL Desk
Comment=Desktop GUI for microHIL hardware controller
Exec=microhildesk
Icon=microhildesk
Terminal=false
...
def main() -> int:
    ...
    """
    print("Starting Debian package creation for microhildesk...")

    # 1. Validation checks
    if not os.path.exists(BINARY_PATH):
        print(f"Error: Executable not found at '{BINARY_PATH}'.")
        print("Please build the application first by running 'make release' or 'make' in sw/microhildesk/build/")
        return 1
    ...
    package_name = f"microhildesk_{version}_{arch}"
    ...
    pkg_binary_path = os.path.join(bin_dir, "microhildesk")
    ...
    if os.path.exists(LOGO_PATH):
        pkg_icon_path = os.path.join(pixmaps_dir, "microhildesk.png")
    ...
    create_desktop_entry(os.path.join(apps_dir, "microhildesk.desktop"))
    os.chmod(os.path.join(apps_dir, "microhildesk.desktop"), 0o644)
